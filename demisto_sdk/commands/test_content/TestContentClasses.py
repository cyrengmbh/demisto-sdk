...
class TestPlaybook:
    ...
    def delete_incident(self, client: DefaultApi, incident_id: int) -> bool:
        """
...
class BuildContext:
    def __init__(self, kwargs: dict, logging_module: ParallelLoggingManager):
        ...
        self.secret_conf_path = kwargs['secret']
        ...
        self.specific_tests_to_run = self._parse_tests_list_arg(kwargs['tests_list'])
    ...
    def _get_server_numeric_version(self) -> str:
        ...
        numeric_version = self._extract_server_numeric_version(instances_ami_name, default_version)
        self.logging_module.info(f'Server version: {numeric_version}', real_time=True)
        return numeric_version

    @staticmethod
    def _extract_server_numeric_version(instances_ami_name, default_version):
    ...
    def _print_investigation_error(self):
        try:
            res = demisto_client.generic_request_func(
                self=self.client,
                method='POST',
                path='/investigation/' + urllib.parse.quote(self.incident_id),
                body={"pageSize": 1000})
    ...
    def _execute_tests(self, queue: Queue) -> None:
        ...
        while queue.unfinished_tasks:
            try:
                ...
                continue
            self._configure_new_client()
            if TestContext(self.build_context, test_playbook, self.client).execute_test(self.proxy):
